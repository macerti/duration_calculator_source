...
- **UPDATE 2026-09-15 (fifty-ninth session)**: parameter admin — **backend built and tested**. Surveyed all 9 `engine/*.php` files; `parameter_sets` already covered nearly everything, the one real hardcoded-formula gap (`engine/nae.php`'s `0.75`/`4`) is now a `naeCoefficients` parameter section. Full versioned admin API (`GET`/`PUT /admin/parameters`, `.../versions`, `.../activate`) on top of the pre-existing `parameterSetRepo.php`. 14 new HTTP tests, `131/131` total, including a full edit→verify-calculation-changed→rollback→reverify cycle. **`AdminParametersScreen.tsx` (the actual browser editor) is still unstarted** — that's the remaining piece of this item. Also still open: whether "operations, steps" (not just numbers) should become editable too — flagged to Mahdi, not assumed.
- **UPDATE 2026-09-15 (sixtieth session)**: **`AdminParametersScreen.tsx` built** — v5.7.0. Real editable fields for `naeCoefficients`/`reportWritingPercent`/`rounding.nearest`/`aggregateFactorCaps`, a version-history list with one-tap rollback. Deliberately still read-only: `iafDurationTables`/`factorCatalogue`/`synergyGrid`/`naceTable` (shown as counts only) — a real table editor for those is a separate, larger follow-up. `tsc`/`expo export`/`make build-deploy` all clean; full backend suite re-confirmed unchanged. Live click-through still pending, same as every frontend feature in this project.
...
- **UPDATE 2026-09-08 (forty-third session)**: `TrackerItem`/`TrackerUpdate` interfaces + all 7 client methods added to `useAdminApi.ts`, `tsc --noEmit` clean — client-side plumbing only, `AdminTrackerScreen.tsx` itself still not written. Full detail: `docs/DEV_STATUS.md`'s forty-third-session entry.
- **UPDATE 2026-09-08 (forty-fourth session) — `AdminTrackerScreen.tsx` WRITTEN, wired in, and fully verified. This item is functionally complete and live.** List with status/type/priority filters, detail panel (ordered update history, editable status/priority, "log an update" form), inline create, two-tap delete — built directly against the already-tested API, mirroring `AdminAnnotationsScreen.tsx`. Uses `maxWidth={1100}` deliberately (not the narrower 800 `AdminAnnotationsScreen` uses) per DEBT-002's own note not to bake that mistake into a new screen. Wired into `App.tsx` (route + `RootStackParamList`) and `ProfileScreen.tsx` (new `manage_tracker`-gated button, `profile-admin-tracker-button`). **Full verification, fresh**: `tsc --noEmit` clean, `expo export --platform web --clear` → **559 modules** (558 baseline + 1, exactly as the forty-second session's hand-off predicted), `make build-deploy` 4/4, `check-repo-hygiene.sh` 4/4. Backend baseline independently reconfirmed unchanged: `smoke_test.php` **24/24**, `http_api_test.php` **82/82**. Version bumped 5.2.0 → **5.3.0** — this is the first version where the tracker is genuinely user-reachable, so this bump follows the project's normal convention (not a deviation, unlike the forty-first session's 5.2.0 bump). **Still open**: a `tracker_items` row for FEAT-010 itself (this feature) — attempted via the live API this session but not completed. Verified the actual mechanism while investigating (`userRepo.php`'s `countActiveUsersWithRole()`): registration only auto-bootstraps the new user as `administrateur` when zero active administrateurs exist yet — a real rule, not a bug. This session's DB already had one (the CI test suite's own registered admin, from this same session's earlier `make test-http` run), so the new test registrant correctly landed as default `utilisateur` and couldn't create the tracker row. Next session: either seed the row directly via a migration, or grant the test user `manage_tracker` via `setUserRole`/direct DB update before using the live API.
- **UPDATE 2026-09-08 (forty-fifth session) — hand-off item 1 CLOSED.** Added migration `006_seed_feat010_self.sql` (`INSERT IGNORE`, matching `005`'s own convention) rather than fighting the live-API auth path — sidesteps the bootstrap-role mechanism entirely. `tracker_items` now has **14 rows**. This surfaced a real, worth-recording regression: two hardcoded `count === 13` assertions in `tests/http_api_test.php` (list-all and back-to-baseline-after-delete) broke against the new 14-row baseline — not a code bug, just a stale test expectation, same category as the forty-first session's permission-count-6-to-7 fix. Both updated to 14. **Also independently caught and fixed a testing-process mistake within this same session**: ran `make test-http` twice in a row against the same already-populated database without resetting between runs, producing a cascade of 35 false-positive 403 failures (the suite's own registrant no longer bootstraps as admin on a non-fresh DB, for the exact reason described just above) — recognized as environment staleness rather than a real regression, database dropped and rebuilt, single clean run captured to a file rather than re-invoked live. **Final confirmed state**: fresh-DB `migrate.php` (6/6 applied, idempotent) → `seed.php` → `smoke_test.php` **24/24** → `http_api_test.php` **82/82**, zero failures. No frontend files touched this session, so no frontend re-verification performed (consistent with this project's own standing rule — only re-run what the diff could plausibly affect). No version bump — a data-seed migration and a test-assertion fix aren't user-reachable functionality on their own. Full detail: `docs/DEV_STATUS.md`'s forty-fifth-session entry. No live click-through from a real browser/device yet (same standing sandbox limitation as every frontend feature in this project). Full detail: `docs/DEV_STATUS.md`'s forty-fourth-session entry.
