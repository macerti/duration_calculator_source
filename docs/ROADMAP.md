...
#### 10. FEAT-006 — In-app admin annotation/comment tool (NEW, requested 2026-09-06 — NOT yet speced)
- **Category:** Testing & Feedback Tooling / Core Administration
- **Status:** REQUESTED, recorded close to verbatim in `docs/DEV_STATUS.md`'s 2026-09-06 (thirty-second session) entry — no design or feasibility work done yet, same treatment FEAT-005/local-accounts got when first requested. **Per Mahdi's explicit instruction, this is the next feature priority once currently-open bugs are closed — ahead of items 1/2/6/7 above.**
- **Objective (as requested):** admin-privileged users right-click (desktop) or tap-and-hold (mobile) anywhere in the app to open a custom contextual menu; besides normal contextual actions, an "add comment" action pins a timestamped, app-version-stamped comment to the exact {x,y} position AND to the specific UI element there (e.g. the header). Comments export to a document (with timestamps + app version) meant to be directly usable by other developers — and by Claude — to action as bugs/feature requests.
- See `docs/DEV_STATUS.md`'s thirty-second-session entry for the open technical questions to resolve before/while writing this item's full spec (element-tagging strategy, native long-press capture, scope of "basic contextual menu stuff", possible overlap with item 1's export tooling).
