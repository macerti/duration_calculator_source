...
# src/backend/tests added to the pattern 2026-09-12 (fifty-fourth session,
# DEBT-004) when that directory moved to the top-level tests/backend/ —
# same reasoning as the other three pre-restructure names above.
...
STALE_PATTERN='audit-app|audit-mobile|duration-calculator-php|audit-engine|src/backend/tests'
...
KNOWN_EXCEPTIONS="src/backend/data/parameters.php scripts/check-repo-hygiene.sh scripts/check-deploy-artifact.sh src/backend/db/migrations/008_extract_buglog_history.sql src/backend/db/migrations/005_seed_tracker_backlog.sql src/backend/db/migrations/012_feat008_slice1_status.sql docs/TRACKER_SNAPSHOT.json src/backend/db/migrations/015_bug053_and_feat012_verified.sql src/backend/db/migrations/017_debt004_progress_and_triage.sql src/backend/db/migrations/018_archive_devstatus_into_session_log.sql src/backend/db/migrations/019_sync_buglog_deltas_before_archive.sql src/backend/db/migrations/020_archive_buglog_preamble.sql src/backend/db/migrations/022_archive_parallel_session_addendum.sql"
...
# 005_seed_tracker_backlog.sql and 012_feat008_slice1_status.sql: same
# "explaining history" situation — both store tracker_items text
# (DEBT-004's own original description, and a "dependencies" field) that
# correctly named src/backend/tests/ as current at the time each was
# written, before that directory moved (2026-09-13/14, DEBT-004 itself).
...
# 2026-09-12 fifty-sixth session) and 017_debt004_progress_and_triage.sql
# (the same class of issue, caught proactively this time rather than by
# CI failing first): each migration's own technical_description explains
# what this very check looks for by spelling out the old path names
# verbatim — self-triggering the check it was describing. BUG-053's
# instance passed locally when first written because the file was still
# untracked (git ls-files, which this check relies on, doesn't see an
# unstaged file) — committing it made it visible to the same check on
# the next run, which is what CI correctly caught and that session's own
# earlier local verification missed. Same "explaining the mechanism, not
